...
class FTPResponseUtilImpl : ErrorUtil
{
  ...
  private:
    ...
    static void transform(WaitingChar *pCurrState, CharEvent event, WaitingChar *pLastState /*NULLAble*/);
    static CharEvent eventFromChar(char ch);
};
void FTPResponseUtilImpl::transform(WaitingChar *pCurrState, CharEvent event, WaitingChar *pLastState)
{
...
std::vector<std::string> FTPResponseUtilImpl::getResponse(int connfd)
{
    char buf[4096];
...
FTPResponseUtilImpl::CharEvent FTPResponseUtilImpl::eventFromChar(char ch)
{
    CharEvent event;
    if (isdigit(ch))
        event = DIGIT;
...
std::vector<std::string> FTPResponseUtil::getResponse(ControlFd connFd)
{
    return FTPResponseUtilImpl::getResponse(connFd);
}

std::vector<std::string> FTPResponseUtil::echoResponse(ControlFd connFd, std::ostream &out)
{
 ...
 */
void FTPResponseUtil::PASVResponse(ControlFd connFd, std::string &ip, uint16_t &port)
{
    std::string response;
    {
