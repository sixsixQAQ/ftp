...
void
_userImpl_ (const ControlFd &fd, const std::string &userName)
{
...
void
_passwordImpl_ (const ControlFd &fd, const std::string &passwd)
{
...
void
passiveImpl (Toggle &mode)
{
	mode.toggle();
}

void
pwdImpl (const ControlFd &fd)
{
...
void
deleteImpl (const ControlFd &fd, const std::string &filePath)
{
...
void
rmdirImpl (const ControlFd &fd, const std::string &dirPath)
{
...
static void
withPassiveMode (const ControlFd &fd, std::function<void (DataFd dataFd)> callback)
{
...
std::string
lsImpl (const ControlFd &fd, const Toggle &isPassive, const std::string &path)
{
	if (isPassive) {
		...
		withPassiveMode (fd, [&] (DataFd dataFd) {
			...
			retStream << IOUtil::readAll (dataFd);
		});
		FTPResponseUtil::echoResponse (fd, retStream);
...
void
cdImpl (const ControlFd &fd, const std::string &path)
{
...
long
sizeImpl (const ControlFd &fd, const std::string &path)
{
	FTPUtil::sendCmd (fd, {"SIZE", path});
	std::stringstream stream;
	return FTPResponseUtil::sizeResponse (fd);
}