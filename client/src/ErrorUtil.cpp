...
std::string ErrorUtil::getLastError()
{
    return m_errorString + "," + m_errorFile + "," + m_errorFunction + "," + std::to_string(m_errorLineNo);
}

void ErrorUtil::setError(const std::string &error, const std::string &file, const std::string &function, int lineNo)
{
    m_errorString = error;
    m_errorFile = file;
